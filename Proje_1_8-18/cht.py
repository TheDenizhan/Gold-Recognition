...
for i, col_val in enumerate(col_names):
    ...
    outliers = x[(x - x.mean()).abs() > 3 * x.std()]
...
target_col = 'GLD'
data_preprocessed['Date'] = pd.to_datetime(data_preprocessed['Date'], errors='coerce')
data_preprocessed['Date'] = data_preprocessed['Date'].dt.strftime('%Y-%m-%d')  # Formatı dönüştür
